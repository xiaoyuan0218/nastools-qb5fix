...
if [ "${NASTOOL_AUTO_UPDATE}" = "true" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -d /nas-tools-patches ]; then
            echo "正在应用自定义补丁..."
            cp -f /nas-tools-patches/app/message/client/feishu.py /nas-tools/app/message/client/feishu.py 2>/dev/null
            cp -f /nas-tools-patches/web/static/img/feishu.png /nas-tools/web/static/img/feishu.png 2>/dev/null
            cp -f /nas-tools-patches/app/conf/moduleconf.py /nas-tools/app/conf/moduleconf.py 2>/dev/null
            cp -f /nas-tools-patches/third_party/qbittorrent-api/qbittorrentapi/auth.py /nas-tools/third_party/qbittorrent-api/qbittorrentapi/auth.py 2>/dev/null
            echo "补丁应用完成"
        fi
        # ===== 补丁恢复完毕 =====
        # Python依赖包更新
